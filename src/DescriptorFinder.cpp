...
std::vector<double> DescriptorFinder::orientation(const Image &imgX, const Image &imgY, const Point &ip, const int r) {
    int baskets = 36;
    double part = 2*M_PI / baskets;
    std::vector<double> basks;
    basks.resize(baskets);
    Kernel gauss = Kernel::gauss(5);
    for (int j = -r;j < r;j++) {
        for (int k = -r;k < r;k++) {
            ...
            //double valGrad = sqrt(valX * valX + valY * valY)*gauss.getItem((k+r)+2*r*(j+r));
            double valGrad = sqrt(valX * valX + valY * valY);
    ...
    peaks.push_back(idx * part);
    if(idx!=indPrev && basks[idx] / basks[indPrev] > 0.8){
        double l= basks[indPrev - 1];
        double r = basks[indPrev + 1];
        peaks.push_back(indPrev * (part+(l - r) / (((l + r) - 2 * basks[indPrev]))));
...
std::vector<std::vector<double>> DescriptorFinder::findDescriptorsWithRotate(const Image &img, const std::vector<Point> &pts,
    ...
    double part = 2 * M_PI / baskets;
    double diam =2*r;
   Kernel gauss = Kernel::gauss(50);
    for (Point ip: pts) {
        std::vector<double> res;
        res.resize(barChars* baskets);
        std::vector<double> peaks = orientation(imgX, imgY, ip, r);
        for (double orient : peaks) {
            for (int j = -r + 1; j < r; j++) {
                for (int k = -r + 1; k < r; k++) {
                    double valX = imgX.getValBlack(ip.x + j, ip.y + k);
                    double valY = imgY.getValBlack(ip.x + j, ip.y + k);
                    ...
                    int neighborIdx = idx * baskets +
                                      (((int) (anglNeighbor / part)) + 1 < baskets ? ((int) (anglNeighbor / part)) + 1
                                                                                   : 0);
